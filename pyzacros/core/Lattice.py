...
class Lattice:
    ...
    * ``cell_vectors`` -- Define the unit vectors. Optional
    ...
    def plot(self, pause=-1, show=True, color=None, ax=None, close=False, show_sites_ids=False, file_name=None, block=False):
        """
        ...
        *   ``file_name`` -- Saves the figure to the file ``file_name``. The format is inferred from the extension, and by default, ``.png`` is used.
        ...
        site_types_1 = list(set(self.site_types))
        site_types_1.sort()
        
        for i,st_i in enumerate(site_types_1):
