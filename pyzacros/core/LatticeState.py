...
class LatticeState:
    ...
    *   ``add_info`` -- A dictionary containing additional information. For example, ``self.add_info['time']`` will be used as part of the title in the figure generated by the function ``plot()``.
    ...
    def plot(self, pause=-1, show=True, ax=None, close=False, show_sites_ids=False, file_name=None, block=False):
        """
        ...
        *   ``file_name`` -- Saves the figure to the file ``file_name``. The format is inferred from the extension, and by default, ``.png`` is used.
        ...
        colors = ['red', 'green', 'blue', 'magenta', 'navy', 'maroon','darkorange','gold','dodgerblue','lawngreen','sandybrown','salmon',
                  'dimgray','brown','sienna','peru','tan','olive',]
        ...
        for i,sym_i in enumerate([item.symbol for item in items]):
            ...
            for (x,y),site_type,sym in zip(self.lattice.site_coordinates,self.lattice.site_types,symbols):
                if( sym == sym_i ):
                    ...
                    order = None
                    lSpecies = None
                    for c, sp in enumerate(self.surface_species):
                        if( sp.symbol == sym_i ):
                            lSpecies = sp
                            order = c
                            break

                    imarkers =  site_types.index(site_type) 

            if( len(xvalues)>0 ):
                ax.scatter(xvalues, yvalues, 
                           color=colors[order], 
                           marker=markers[imarkers],
